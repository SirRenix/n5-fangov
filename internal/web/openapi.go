...
func (s *Server) routeTable() []route {
	...
	return []route{
		...
		{Method: "GET", Path: "/api/history", Handler: s.getHistory, Class: classFiltered, Scope: scopeRead,
			...
			Params:    []paramSpec{{Name: "minutes", Type: "integer", Description: "span in minutes, 1..10080; the tier follows the span: raw up to 2 h, 1-min means up to 24 h, 5-min means beyond (default 120)"}, {Name: "since", Type: "integer", Description: "only points with ts > since (unix seconds)"}},
		...
		{Method: "GET", Path: "/api/schedules", Handler: s.getSchedules, Class: classProtected, Scope: scopeRead,
			Summary: "Preset schedules: entries, the active one, next and last switch, timezone", Response: "Schedules", Responses: map[int]string{200: "schedule status", 501: "no scheduler"}},
...
func openAPISchemas() map[string]any {
	...
	return map[string]any{
		...
		"Channel": object("One regulated channel", []string{"name", "pwm", "sensor", "temp", "duty", "target", "rpm", "mode"}, map[string]any{
			...
			"ceiling":     integer("effective ceiling in °C: the sensor kind's built-in one (HDD 65, SSD 85, CPU 100), lowered by [[channel]] ceiling; signed in only"),
			"ceiling_hit": boolean("the channel is in the ceiling state (255, mode critical) until the reading is 3 °C below the ceiling; signed in only"),
		}),
		...
		"Version": object("GET /api/version", []string{"name", "version", "prerelease", "tls", "auth", "limits"}, map[string]any{
			...
			"limits": object("validation bounds for clients", nil, map[string]any{
				...
				"ceiling_min":           integer("lowest [[channel]] ceiling (°C); the upper bound is the sensor's built-in ceiling"),
			}),
		}),
