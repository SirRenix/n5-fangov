...
func TestStaticIndex(t *testing.T) {
	...
	if len(r.body) > 72*1024 {
		t.Errorf("app.js is %d bytes, budget 72 KB (DESIGN v0.3.0-beta)", len(r.body))
	}
	...
	for _, want := range []string{"since=", "b.lines", "cfg.channel", "warnings", "/api/log/export", "/api/config/export", "/api/config/import", "b.source", ".tls", "/api/tls/regenerate", "/api/tls/upload", "/api/tls/reset", "cert.cer", "force_required", "c.warnings", "c.fallback", "Math.ceil((new Date(iso)"} {
