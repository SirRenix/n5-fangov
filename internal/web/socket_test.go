...
func TestUnixSocketRoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "n5-fangov-web")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dir)
	sock := filepath.Join(dir, "n5-fangov.sock")
	...
	c := ipc.Client(sock)
	res, err := c.Get("http://n5-fangov/api/version")
	if err != nil {
		t.Fatal(err)
	}
	...
	req, _ := http.NewRequest("PUT", "http://n5-fangov/api/override/cpu", strings.NewReader(`{"duty":77}`))
	res, err = c.Do(req)
	if err != nil {
		t.Fatal(err)
	}
