...
$('#cv-apply').addEventListener('click', async () => {
	...
	try { const r = await api('/api/config', { method: 'PUT', body, headers: { 'Content-Type': 'application/toml' } });
		n.hidden = r.status !== 202; n.className = 'notice'; n.textContent = 'Saved — restart required (channel set or profile changed): systemctl restart pvefand';
		toast(r.status === 202 ? 'Config written, restart required' : 'Curves applied', 'ok'); await loadConfig(); loadEditor();
