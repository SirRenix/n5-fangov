...
|---|---|---|
| `n5-fangov check` exits 1: `profile "…" not detected` / `setup` finds no profile | the hwmon device is not there: module not loaded, or (N5 Pro) `experimental_write=1` missing. Not a cause: a different `hwmonN` than last boot; the daemon finds the device by name | `n5-fangov detect`; `lsmod \| grep minisforum`; `cat /etc/modprobe.d/minisforum-n5-it5571.conf`; `modprobe minisforum_n5_it5571`; `ls /sys/class/hwmon/*/pwm1`; then `systemctl start n5-fangov` — [Driver options](02-kernel-driver.md#driver-options-and-autoload) |
...
| `status` shows a channel in `sensor-error` | its sensor is unresolved or unreadable. N5 Pro `hdd` channel: the `drivetemp` module is not loaded (nothing loads it by default), or the box has no SATA drives | `lsmod \| grep drivetemp`; `modprobe drivetemp` and `/etc/modules-load.d/drivetemp.conf` ([Drive temperatures](02-kernel-driver.md#drive-temperatures)). Without HDDs it is expected; the channel sits at its stop duty. Otherwise `sensors`, `ls /sys/class/hwmon/*/name`, and the sensor id in the config |
| Browser shows a certificate warning | the automatic certificate is not trusted yet | Settings → *Certificate* → *Download .crt/.cer* → trust it per *How to trust* → reload; or `n5-fangov cert export` ([The certificate](08-https-security.md#the-certificate)) |
| Certificate still untrusted after importing the `.cer` on Windows | the wizard's default store choice put it into the wrong store; or the browser was not restarted; or it is Firefox, which keeps its own store | re-import with *Place all certificates in the following store* → *Trusted Root Certification Authorities* ([Windows walkthrough](08-https-security.md#windows-walkthrough)); close every browser window, reopen; Firefox: Settings → Certificates → Authorities → Import |
| Still a warning after the import, and the store already holds an entry named like the host | an **old** certificate with the same name (`CN=<host name>`) is in the store: after `uninstall.sh --purge` + reinstall, *Regenerate…* with a new key or an upload the daemon serves a different certificate under the same name | compare the fingerprint in Settings → Certificate with the store entry; delete the old entry, import the new file, restart the browser ([A new certificate with the same name](08-https-security.md#the-certificate)) |
| Cannot find the certificate in a store | the entry is named after the host (`n5host`), not "n5-fangov" | look for the host name under *Issued To*; the *subject* row of Settings → Certificate shows the exact name |
| Forgot the dashboard password | root on the host is the only recovery path | `n5-fangov passwd` (same `--user` / `--password-file` / `--password -` flags as `setup`), then `systemctl restart n5-fangov` ([Setup](03-setup.md#change-user-or-password-later)) |
...
| Alerts do not arrive | transport degraded (`pve` without `PVE::Notify`, `mail` without `mail(1)`), cooldown active, PVE matcher routes elsewhere, or (mail path) the sandbox blocks `postdrop` | Alerts page → *Send test alert* (reports the delivery error); `n5-fangov alerts status`; `journalctl -u n5-fangov \| grep ALERT` (`ALERT[kind]: …` = raised, `ALERT[kind] sent via <transport>: …` = handed to the transport; only the first → the sink never ran, `alert: …` after it → delivery failed); [Alert delivery under the sandbox](08-https-security.md#alert-delivery-under-the-sandbox) for `mail(1)` |
...
| Notice `restart required` after Apply/Import/Preset | the channel set or the profile changed; `[web]`/`[log]` keys changed by import | `systemctl restart n5-fangov` (the daemon runs the old set until then) |
| Dashboard banner `Connection to the daemon lost` | daemon restarting or down; the fans stay on the daemon side (failsafe on exit) | `systemctl status n5-fangov`; `journalctl -u n5-fangov -n 50` |
...
| A `[[schedule]]` entry did not switch the preset | the preset name does not exist (checked at the switch), the window's day or time is not what you think (host **local** time; `to < from` crosses midnight), or something applied another preset after the switch; the scheduler acts on transitions only | Schedules page (status card: active, next, last switch with its error, timezone; a missing preset is flagged in its row) / `GET /api/schedules`; Alerts page: kind `schedule`; `journalctl -u n5-fangov \| grep schedule:`; `timedatectl` ([Schedules](06-configuration.md#schedules)) |
| Script gets `401` with a token that worked | the token expired or was revoked (`web: bearer token rejected from <ip>: expired\|unknown` in the journal); or `auth` is `none`, where a Bearer header is ignored | `n5-fangov token list` (expiry column); create a new one; `GET /api/session` with the token shows `via` and `scope` ([API tokens](08-https-security.md#api-tokens)) |
| Script gets `403` | the endpoint is outside the token's scope (`required` in the answer), or a token on a session-only endpoint (`/api/tokens*`, `/api/account/*`), or a cookie/Basic caller without `X-N5-Fangov-Csrf: 1` | create a token with the scope the answer names; tokens and account changes go through the dashboard or Basic auth ([Scopes](12-api.md#scopes)) |
...
`n5-fangov check` walks the list the unit runs as `ExecStartPre` before every start:
config (invalid values that fell back to their default, the misconfigured `auth`
field), profile detection, pwm writability, sensors, the emergency hook
(`/etc/n5-fangov/emergency.sh`: `ok`, `absent` or `refused: <why>`; a warning only with
`emergency = true`), tls, log, dkms (module for the running kernel). Exit 1 means
...
shifts older files up and drops the oldest. Directory `0750`, file `0640`. `file` must
be a plain absolute path under `/var/log/` (no `..`); anything else falls back to the
default with a warning, and an existing target that is not a regular file (symlink,
device, directory) is refused. `[log]` changes take a restart.
...
a file it falls back to the journal and cannot clear. Log lines never carry secrets;
hashes, tokens and passwords are redacted at the source. A status line every
`[daemon] log_every` cycles shows what the controller is doing.
