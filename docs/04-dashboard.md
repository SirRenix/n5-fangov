...
Settings → Display, stored in this browser. In the rail the same panel icon sits directly
under the logo — the toggle stays in the sidebar in both states, nothing moves into the
page header. Between 700 and 1099 px the rail is forced (the sidebar would leave the
header no room): the rail has no toggle and the logo's tooltip says *Sidebar collapses
below 1100 px*; the stored choice applies from 1100 px. The footer shows the daemon's version.
